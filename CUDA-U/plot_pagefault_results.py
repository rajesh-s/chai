...
def plot_all_benchmarks(data, output_dir, system_name="System"):
    """Generate page fault plots for all benchmarks."""
...
def plot_all_comparisons(data1, data2, output_dir, name1="GH200", name2="x86+H100"):
    """Generate comparison plots for all common benchmarks."""
    os.makedirs(output_dir, exist_ok=True)
...
def plot_summary_bar_chart(data1, data2, output_dir, name1="GH200", name2="x86+H100"):
    """
    Create a summary bar chart comparing total page faults across all benchmarks.
    ...
    bars1 = ax.bar(x - width/2, totals1, width, label=name1, color=COLORS_SYSTEM1[3])
    bars2 = ax.bar(x + width/2, totals2, width, label=name2, color=COLORS_SYSTEM2[3])
    ...
    for i, (t1, t2) in enumerate(zip(totals1, totals2)):
        if t1 > 0:
    ...
    print(f"  Saved summary bar chart")
